...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("trading.log"), logging.StreamHandler(sys.stdout)],
)
...
class TradingSystem:
    ...
    def __init__(self, config: TradingConfig):
        ...
        self.monitoring = MonitoringSystem(
            database=self.database, email_config=config.email_config
        )
        ...
        self.trading_client = Client(
            api_version=APIVersion.V2, is_sandbox=config.is_sandbox
        )
        ...
        self.portfolio_manager = PortfolioManager(
            total_budget=config.budget, stock_configs=stock_configs
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--budget", type=float, required=True, help="Total trading budget"
    )
    parser.add_argument("--symbols", nargs="+", required=True, help="Trading symbols")
    parser.add_argument(
        "--base-quantity", type=int, required=True, help="Base quantity per trade"
    )
    parser.add_argument(
        "--update-interval", type=int, default=60, help="Update interval in seconds"
    )
    parser.add_argument(
        "--strategy",
        choices=["technical", "simple"],
        required=True,
        help="Trading strategy",
    )
    parser.add_argument("--buy-price", type=float, help="Buy price for simple strategy")
    parser.add_argument(
        "--sell-price", type=float, help="Sell price for simple strategy"
    )
    parser.add_argument("--live", action="store_true", help="Run in live mode")
    parser.add_argument(
        "--email-config", type=str, help="Path to email configuration file"
    )
