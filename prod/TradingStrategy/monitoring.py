...
class MonitoringSystem:
    ...
    def check_portfolio_health(self) -> List[str]:
        ...
        if self.daily_start_value > 0:
            daily_return = (latest.total_value - self.daily_start_value) / self.daily_start_value
            if daily_return < -self.alert_config.daily_loss_threshold:
                alerts.append(
                    f"Daily loss threshold exceeded: {daily_return:.2%}"
                )
        ...
        if latest.total_value > self.peak_portfolio_value:
            ...
            drawdown = (self.peak_portfolio_value - latest.total_value) / self.peak_portfolio_value
    ...
    def check_risk_metrics(self) -> List[str]:
        ...
        if latest["volatility"] > self.alert_config.volatility_threshold:
            alerts.append(
                f"High volatility: {latest['volatility']:.2%}"
            )
