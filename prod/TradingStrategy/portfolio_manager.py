...
class PortfolioManager:
    ...
    def __init__(
        ...
        logger.info(f"Tracking {len(stock_configs)} stocks: {', '.join(str(s) for s in stock_configs)}")
    ...
    def _should_exit_position(self, position: StockPosition) -> bool:
        ...
        if position.position_type == "LONG":
            ...
            return (position.current_price <= position.stop_loss or 
    ...
    def analyze_portfolio(self) -> List[TradingStrategyData]:
        ...
        for symbol, position in list(self.positions.items()):
            if self._should_exit_position(position):
                ...
                trade_details = TradingStrategyData(
                    ...
                    transaction_type=BaseTransactionType.SELL if position.position_type == "LONG" else BaseTransactionType.BUY
                )
        ...
        for symbol, config in self.stock_configs.items():
            ...
            trade_details = technical_analysis_strategy(
                strategy_config,
                Decimal(str(self.price_history[symbol][-1]))
            )
            ...
            if trade_details.transaction_type:
                ...
                new_position = StockPosition(
                    ...
                    position_type="LONG" if trade_details.transaction_type == BaseTransactionType.BUY else "SHORT",
                ...
                self.portfolio_config.current_positions[symbol] = current_price * trade_details.quantity
