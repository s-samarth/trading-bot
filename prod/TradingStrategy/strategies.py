...
class PortfolioConfig:
    ...
    max_position_size: float = (
        0.1  # Maximum position size as fraction of total budget (default: 10%)
    )
    risk_per_trade: float = (
        0.02  # Maximum risk per trade as fraction of total budget (default: 2%)
    )
...
def simple_strategy(
    ...
    trade_details.quantity = calculate_volume(
        ...
        (
            ...
            else config.sell_price
        ),
    )

    logger.info(
        f"Signal strength: {signal_strength:.2f}, Trading volume: {trade_details.quantity}"
    )
    return trade_details
...
def technical_analysis_strategy(
    ...
    if len(config.price_history) < config.min_price_history:
        logger.info(
            f"Insufficient price history. Need {config.min_price_history} points, got {len(config.price_history)}"
        )
        return trade_details
    ...
    action, stop_loss, take_profit = generate_trading_signals(
        current_price=float(ltp), indicators=indicators, position=config.position
    )
    ...
    logger.info(
        f"BB Upper: {indicators.bb_upper:.2f} | BB Lower: {indicators.bb_lower:.2f}"
    )
    logger.info(
        f"Action: {action} | Stop Loss: {stop_loss:.2f} | Take Profit: {take_profit:.2f}"
    )
    ...
    if action != "HOLD":
        ...
        if action == "BUY" and float(ltp) < indicators.bb_lower:
            bb_strength = (indicators.bb_lower - float(ltp)) / (
                indicators.bb_middle - indicators.bb_lower
            )
        elif action == "SELL" and float(ltp) > indicators.bb_upper:
            bb_strength = (float(ltp) - indicators.bb_upper) / (
                indicators.bb_upper - indicators.bb_middle
            )
