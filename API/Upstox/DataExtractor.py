...
class DataExtractor:
    def __init__(
        self,
        gzip_file_path="UpstoxData/complete.json.gz",
        ...
        self.complete_data_upstox_path = os.path.join(
            self.current_dir, complete_data_upstox_path
        )
        self.nse_data_eq_path = os.path.join(self.current_dir, nse_data_eq_path)
        self.nse_trading_symbol_to_isin_path = os.path.join(
            self.current_dir, nse_trading_symbol_to_isin_path
        )
    ...
    def extract_nse_eq_data(self, save_data=True):
        ...
        nse_eq_data = [
            ...
            and entry.get("exchange") == "NSE"
            and entry.get("instrument_type") == "EQ"
        ]
        if len(nse_eq_data) == 0:
            print("❌ No NSE_EQ data found.")
            return None
        print(
            f"✅ Extracted NSE_EQ data from {self.complete_data_upstox_path} with {len(nse_eq_data)} entries."
        )

        try:
            symbols = {entry["trading_symbol"] for entry in nse_eq_data}
            assert len(nse_eq_data) == len(
                symbols
            ), "Trading Symbols are not unique in NSE EQ data."
    ...
    def get_nse_trading_symbol_to_isin_map(self):
        ...
        symbol_to_isin = {
            entry["trading_symbol"]: entry["isin"] for entry in nse_eq_data
        }
    ...
    def save_nse_trading_symbol_to_isin_map(self, symbol_to_isin):
        ...
        try:
            ...
            print(
                f"✅ Saved trading symbol to ISIN mapping to {self.nse_trading_symbol_to_isin_path}"
            )
        except Exception as e:
