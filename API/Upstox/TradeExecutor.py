...
from API.Upstox.Constants import Exchange, Validity, TransactionType, ProductType, OrderType, Segment
...
class PlaceOrderData(BaseModel):
    ...
    price: float = 0 # The price at which the order is to be executed. For market orders, this is ignored.
    tag: Optional[str] = None
    disclosed_quantity: int = 0  # The portion of the total order quantity that is visible to the market at any given time. Used by big traders to hide their order size and avoid market impact.
    trigger_price: float = 0 # The price at which the order is triggered. This is used for stop-loss orders. For market orders, this is ignored.
    is_amo: bool = False  # After market order. If True, the order will be placed after the market closes and will be valid for the next trading day.
...
class ModifyOrderData(BaseModel):
    ...
    quantity: Optional[int] = None # The quantity of the order. If not provided, the order will be modified with the same quantity.
    price: float = 0 # The price at which the order was placed
    validity: Validity = Validity.DAY # The validity of the order. If not provided, the order will be valid for the day.
    order_type: OrderType = OrderType.MARKET # The type of order. If not provided, the order will be a market order.
    disclosed_quantity: Optional[int] = None  # If provided, this value must be non-zero.
    trigger_price: float = 0 # If the order is a stop loss order then the trigger price to be set is mentioned here
...
class TradeExecutor:
    ...
    def validate_response(self, response, endpoint):
        ...
        if response.status_code == 200:
            ...
            print(f"Partial success for endpoint: {endpoint}, Response: {response.json()}\n")
            return response.json()
        else:
...
class OrderAPI(TradeExecutor):
    ...
    def place_order(self, order_data: PlaceOrderData):
        ...
        instrument_token = generate_instrument_token(order_data.trading_symbol, order_data.exchange)
        data = {
    ...
    def cancel_multi_order(self, exchange: Optional[Exchange]=None, segment: Optional[Segment]=None, tag: Optional[str]=None):
        """
...
if __name__ == "__main__":
    ...
    modify_order_data = ModifyOrderData(order_id=place_order_response["data"]["order_id"])
    ...
    cancel_order_response = order_api.cancel_order(order_id=place_order_response["data"]["order_id"])
    print(f"Cancel Order Response: {cancel_order_response}\n")

    cancel_multi_order_response = order_api.cancel_multi_order(exchange=Exchange.NSE, segment=Segment.EQUITY)
    ...
    modify_order_data = ModifyOrderData(order_id=place_order_response["data"]["order_ids"][0])
    modify_order_response = order_api_v3.modify_order(order_data=modify_order_data)
    print(f"Modify Order Response: {modify_order_response}\n")

    cancel_order_response = order_api_v3.cancel_order(order_id=place_order_response["data"]["order_ids"][0])
    print(f"Cancel Order Response: {cancel_order_response}\n")
